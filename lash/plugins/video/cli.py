...
def intro(video_path, video_intro, o):
    ...
    intro_clip = VideoFileClip(video_intro)
    composed = concatenate_videoclips([intro_clip, video])
...
def end(video_path, video_end, o):
    video = VideoFileClip(video_path)
    end_clip = VideoFileClip(video_end)
    composed = concatenate_videoclips([video, end_clip])
    filename = get_last(video_path)
    if o:
        composed.write_videofile(f'{video_path.replace(filename, o)}.mp4')
    else:
